...
牌効率は、手牌の構造から計算できる材料を土台とし、公開情報、牌の所在推定の順に利用情報を広げます。論理的・決定的に計算できる材料と、不確実性を伴う推定を区別し、それらをheuristic / ML / NNによる選択へ接続します。

| 段階 | 利用する情報 | 評価する能力 |
| --- | --- | --- |
| 1. 手牌の構造 | 自手・自分の副露 | 向聴数、有効牌、基本的な受け入れ |
| 2. 公開情報による補正 | 自手・河・副露・ドラ表示牌等 | 未見の有効牌枚数、枯れた待ち、手替わり、複数手先の進行 |
| 3. 牌の所在推定による補正 | 公開情報と他家の行動履歴等 | 他家の手牌にある確率・期待枚数、山に残る枚数の推定、それに基づく牌の利用可能性 |

これは能力の発展順序であり、各段階がすべて未実装という意味ではありません。既存のremaining tile inventory、受け入れ・lookahead、belief基盤等は再利用し、実装済み範囲と不足は各repositoryの文書・Issueで確認します。

向聴数は定義を明示した構造上の進行度として扱い、未見枚数や所在推定と区別します。未見枚数は実際の山の枚数ではありません。基本的な受け入れについても、自手・副露等のどこまでを差し引くかをcontractで明示します。段階の追加だけを理由にcanonical向聴数の定義を変更せず、定義変更自体を独立した強化目標にはしません。既存定義と外部referenceとの差は記録し、具体的な判断上の問題が確認された場合に再検討します。

公開情報や所在推定を受け入れへ反映する際は、向聴数による候補の事前除外も評価対象にします。和了牌が残らない聴牌からの向聴戻しや、向聴数を変えずに待ちを改善する手替わりは、単純な向聴改善牌の枚数だけでは評価できません。少数の具体例で候補選別・手替わり・先読みの挙動を確認し、残り枚数の補正だけで解決したとは扱いません。

所在推定では、ツモに使える可能性と、相手が切ることによってロン・鳴きに使える可能性を区別します。推定に基づく一定巡数以内の聴牌・和了確率等を導入する場合も、通常の向聴数とは別の指標として意味を定義します。online判断には合法的な観測情報のみを使い、hidden ground truthはoffline検証に限定します。

各段階の効果は、計算・推定の正確さ、同一局面での判断変化、対局成績、計算時間を分けて確認します。推定精度の向上だけでPolicyの強化を主張せず、可能な限り1つの変更軸ごとに比較します。ルール上の聴牌・立直・和了の合法性はengine側の責務を維持します。
